...
function safeLocatorWithIframeSupport(page, selector) {
  const selectorArr = Array.isArray(selector) ? selector : [selector];
  const safeSelectorArr = selectorArr.map((sel) => {
    if (sel.startsWith("//")) {
      return sel;
...
var StagehandExtractHandler = class {
  ...
  textExtract(_0) {
    return __async(this, arguments, function* ({
      ...
      const {
        selectorMap
      } = yield this.stagehand.page.evaluate(() => window.processAllOfDom());
      this.logger({
        category: "extraction",
  ...
  return __async(this, null, function* () {
    ...
    if (env === "BROWSERBASE") {
      ...
      if (browserbaseSessionID) {
        ...
        const session = yield browserbase.sessions.create(__spreadValues(__spreadValues({
          projectId
        }, browserbaseSessionCreateParams), shouldUseUnsafeMode ? {
          browserSettings: {
